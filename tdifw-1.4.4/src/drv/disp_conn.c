...
tdi_connect(PIRP irp, PIO_STACK_LOCATION irps, struct completion *completion)
{
	...
	//远程地址直接在param->RequestConnectionInformation->RemoteAddress中直接拿
	//主要是本地地址要从HASH表里查询，HASH表中的本地地址是在create里查询出来的
	...
	//根据连接对象irps->FileObject在HASH表中找到连接对象结构体
	...
	if (ipproto == IPPROTO_TCP) {
		 ...
		 */
		//UDP，地址对象和连接对象是一样的，都是irps->FileObject
	...
	//quick_filter就是将request里的地址和端口和rule规则进行匹配

	result = quick_filter(&request, &rule);//这里就可以替换弹窗
