...
#ifndef USE_TDI_HOOKING
	//创建过滤设备：g_tcpfltobj g_tcpfltobj g_ipfltobj
	//用来绑定到tcp,udp,rawip设备上，拦截经过的IRP包
...
DeviceDispatch(IN PDEVICE_OBJECT DeviceObject, IN PIRP irp)
{
	...
	if (DeviceObject == g_tcpfltobj || DeviceObject == g_udpfltobj ||
		DeviceObject == g_ipfltobj) {//主要为了判断是我们自己发的还是别人发的
		...
		case IRP_MJ_CREATE:		/* create fileobject */
			...
			//下发create irp，result用来决定是下发放行还是阻止该IRP
			//而ip地址和端口是在completion.routine里完成的,必须要等待IRP完成，因为这时候连接对象和地址对象还没创建出来，所以在下面完成例程里面拿我们想要的数据，这两个完成例程，
			//completion.routine = tdi_create_addrobj_complete，一个负责下发
			//completion.context = query_irp，一个负责查询，在IRP_MJ_CREATE完成之后，自己创建一个查询IRP，在创建的这个IRP完成之后，在完成例程里拿到IP地址和端口，然后保存到我们的HASH表中，
			//一个地址对象和一个连接对象，因为只有在create时候才有地址对象，其他查询时候需要连接对象去找地址对象，才能拿到里面存的IP地址和端口。
		...
		case IRP_MJ_DEVICE_CONTROL:
			...
			if (KeGetCurrentIrql() == PASSIVE_LEVEL) {
				 ...
				 */
				status = TdiMapUserRequest(DeviceObject, irp, irps);//把IRP_MJ_DEVICE_CONTROL映射为IRP_MJ_INTERNAL_DEVICE_CONTROL
			
			} else
				status = STATUS_NOT_IMPLEMENTED; // set fake status

			if (status != STATUS_SUCCESS) {
				//IOCTL_TDI_QUERY_DIRECT_SEND_HANDLER,这个需要重新设置
				//用新的new_TCPSendData函数来替换原来的g_TCPSendData
				//见下面的替换代码
		...
		case IRP_MJ_INTERNAL_DEVICE_CONTROL: {
			 ...
			 */
			//此处处理主功能号IRP_MJ_INTERNAL_DEVICE_CONTROL对应的各种
			//次功能号，这些次功能号的处理函数都在g_tdi_ioctls数组中。
			//见dispatch.c文件和event.c文件
			int i;
	...
	} else if (DeviceObject == g_devcontrol) {//这就是我们自己的控制设备了。
