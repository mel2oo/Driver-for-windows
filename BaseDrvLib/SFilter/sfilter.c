...
LIST_ENTRY g_OperList;	//操作列表，跟踪各种被监控的行为
ERESOURCE  g_OperListLock;

LIST_ENTRY g_WaitList;	//等待环三允许或者阻止的列表
ERESOURCE  g_WaitListLock;

LIST_ENTRY g_PendingIrpList; //环三读操作列表的内容被挂起的IRP
ERESOURCE  g_PendingIrpListLock;

ULONG g_ulWaitID = 0; //等待队列中标志

HANDLE g_hSystemProcID = NULL;
...

{
	...
	g_hSystemProcID = PsGetCurrentProcessId();
    ...
    RtlInitUnicodeString( &nameString, L"\\FileSystem\\Filters\\hipsdrv" );
    ...
    if (status == STATUS_OBJECT_PATH_NOT_FOUND) {
        ...
        RtlInitUnicodeString( &nameString, L"\\FileSystem\\hipsdrv" );
	...
	RtlInitUnicodeString(&linkString, L"\\DosDevices\\hipsdrv");
...
NTSTATUS
IoControlDispatch(PDEVICE_OBJECT lpDevice, PIRP lpIrp)
{
	...
	ULONG							ulCode				= lpIrpStack->Parameters.DeviceIoControl.IoControlCode;
	...
	//获得输入缓冲和长度
	inputBuffer = lpIrp->AssociatedIrp.SystemBuffer;
	inputBufferLength = lpIrpStack->Parameters.DeviceIoControl.InputBufferLength;
	//获得输出缓冲和长度
	outputBuffer = lpIrp->AssociatedIrp.SystemBuffer;
	...
	case IOCTL_SEND_RESULT_TO_R0:
		{
			...
			{
				lpIrp->IoStatus.Information = 0;
				lpIrp->IoStatus.Status = STATUS_INVALID_PARAMETER;
				break;
			}
			lpReply = (RING3_REPLY*)lpIrp->AssociatedIrp.SystemBuffer;
			
			LockWrite(&g_WaitListLock);
			lpWaitEntry = FindWaitEntryByID(&g_WaitList, lpReply->m_ulWaitID);
			...
			lpIrp->IoStatus.Information = 0;
			Status = lpIrp->IoStatus.Status = STATUS_SUCCESS;
		}
		break;
	default:
		break;
...
PendingIrpToList(
		PIRP lpIrp, 
		PLIST_ENTRY lpIrpList, 
		PDRIVER_CANCEL lpfnCancelRoutine)
{
	InsertTailList(lpIrpList, &lpIrp->Tail.Overlay.ListEntry);
	IoMarkIrpPending(lpIrp);//lpIrp to be pended
	//CancelIo from r3  or IoCancelIrp to call  lpfnCancelRoutine
...
NTSTATUS
ReadDispatch(PDEVICE_OBJECT lpDevice, PIRP lpIrp)
{
	NTSTATUS			Status				= STATUS_SUCCESS;
	ULONG				ulInformation		= 0;	
	PIO_STACK_LOCATION	lpIrpStack			= IoGetCurrentIrpStackLocation(lpIrp);
	OP_INFO		*lpFileOpInfoEntry	= NULL;
	LIST_ENTRY			*lpFileOpInfoList	= NULL;
	...
	lpFileOpInfoList = g_OperList.Flink;
	lpFileOpInfoEntry = CONTAINING_RECORD(lpFileOpInfoList, OP_INFO, m_List);
	RemoveEntryList(lpFileOpInfoList);
	UnLockWrite(&g_OperListLock);
	
	RtlCopyMemory(lpIrp->AssociatedIrp.SystemBuffer, lpFileOpInfoEntry, sizeof(RING3_OP_INFO));
	Status = STATUS_SUCCESS;
	ulInformation = sizeof(RING3_OP_INFO);
	
	ExFreePoolWithTag(lpFileOpInfoEntry, 'POIF');
...
NTSTATUS
sfRead(PDEVICE_OBJECT lpDevice, PIRP lpIrp)
{
    NTSTATUS Status = STATUS_SUCCESS;
	
    if (IS_MY_CONTROL_DEVICE_OBJECT(lpDevice))
    {
        Status = ReadDispatch(lpDevice, lpIrp);
...
NTSTATUS
sfDeviceIoControl(PDEVICE_OBJECT lpDevice, PIRP lpIrp)
{
    NTSTATUS				Status			= STATUS_SUCCESS;
    ULONG					ulInfomation	= 0;
    IO_STACK_LOCATION		*lpIrpStack		= IoGetCurrentIrpStackLocation(lpIrp);
    ...
    if (IS_MY_CONTROL_DEVICE_OBJECT(lpDevice))
    {
        Status = IoControlDispatch(lpDevice, lpIrp);
    }
    else if (!IS_MY_DEVICE_OBJECT(lpDevice))
    {
        lpIrp->IoStatus.Status = Status = STATUS_INVALID_PARAMETER;
...
PNAME_CONTROL
AllocateNameControlFromName(
	PUNICODE_STRING NameString
	)
{
    NTSTATUS			Status		= STATUS_SUCCESS;
    PNAME_CONTROL		NameControl = NULL;
    ...
    Status = NLAllocateNameControl( &NameControl, &gSfNameBufferLookasideList );
    ...
    Status = NLCheckAndGrowNameControl(NameControl, NameString->Length); // Allcated >= NameString->Length + sizeof(WCHAR)
	
    if (!NT_SUCCESS(Status))
    {
        NLFreeNameControl(NameControl, &gSfNameBufferLookasideList);
        return NULL;
    }
	
    RtlCopyMemory(NameControl->Name.Buffer,
		NameString->Buffer,
		NameString->Length);
	
    NameControl->Name.Buffer[NameString->Length / sizeof(WCHAR)] = UNICODE_NULL;
    NameControl->Name.Length = NameString->Length;
...
_EXIT:
    if (bSkipped)
    {
		...
		//home work, to update the hash table if succeeded
...
GetFileNameFromObject(
		   PNAME_CONTROL* lppNameControl, 
		   ULONG ulFlag, 
		   FILE_OBJECT* lpFileObject, 
		   PDEVICE_OBJECT lpDevice)
{
    BOOLEAN bCacheName = FALSE;
    NTSTATUS Status = STATUS_SUCCESS;
    PSFILTER_DEVICE_EXTENSION lpDevExt = (PSFILTER_DEVICE_EXTENSION)(lpDevice->DeviceExtension);
	
    Status = NLAllocateNameControl( lppNameControl, &gSfNameBufferLookasideList );
	
    if (NT_SUCCESS(Status) == TRUE) 
    {
        Status = NLGetFullPathName( lpFileObject, *lppNameControl, &lpDevExt->NLExtHeader,
            ulFlag, &gSfNameBufferLookasideList, &bCacheName );
    }
    else
    {
        *lppNameControl = NULL;
...
NTSTATUS
sfClose(PDEVICE_OBJECT lpDevice, PIRP lpIrp)
{
    NTSTATUS				Status			= STATUS_SUCCESS;
    ULONG					ulInfomation	= 0;
    IO_STACK_LOCATION		*lpIrpStack		= IoGetCurrentIrpStackLocation(lpIrp);
...
NTSTATUS
sfCleanup(PDEVICE_OBJECT lpDevice, PIRP lpIrp)
{
    NTSTATUS				Status			= STATUS_SUCCESS;
    ULONG					ulInfomation	= 0;
    IO_STACK_LOCATION		*lpIrpStack		= IoGetCurrentIrpStackLocation(lpIrp);
    ...
    else
    {
		...
		lpNameControl = lpTwoWay->data.lpNameControl;
		...
		R3UserResult = hipsGetResultFromUser(L"清理", lpNameControl->Name.Buffer, NULL, User_DefaultNon);
...
// 	 }
// 
// 	lpTwoWay->data.lpNameControl = NULL;
