...
_CR0 Asm_GetCr0Ex();
_CR4 Asm_GetCr4Ex();
_EFLAGS Asm_GetEflagsEx();
void Asm_SetCr0Ex(_CR0 cr0);
void Asm_SetCr4Ex(_CR4 cr4);

ULONG64 Asm_ReadMsr(ULONG uIndex);
void Asm_ReadMsrEx(ULONG uIndex,PMSR msr);
...
void Vmx_VmLaunch();
void Vmx_VmResume();
void Vmx_VmCall(ULONG uCallNumber);

void Asm_VMMEntryPoint();

void Asm_SetupVMCS();

ULONG Asm_GetGuestReturn();
ULONG Asm_GetGuestESP();

void Asm_AfterVMXOff(ULONG JmpESP, ULONG JmpEIP);
